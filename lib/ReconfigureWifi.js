...
function ifdownup(message) {
  ...
  exec('systemctl disable hostapd', function(error, stdout, stderr) {
    exec('systemctl disable dnsmasq', function(error, stdout, stderr) {
...
module.exports = function(ESSID, password) {
  ...
  var dnsmasq_file = __dirname + '/network_templates/dnsmasq/default'
  var wlan0_file = __dirname + '/network_templates/wlan0/default'
  ...
  fs.writeFileSync('/etc/dnsmasq.conf', fs.sreadFileSync(dnsmasq_file))

  var rc_local_file = __dirname + '/system_templates/rc_local_default'

#!/usr/bin/env node

module.exports = container => {
  if (!['docker', 'balena', 'both', 'none'].includes(container)) {
    console.log("Error: only `docker`, `balena`, `both`, `none` options are supported");
    return;
  }
  var msg;
  var command = __dirname + "/scripts/container_" + container + ".sh";

  if (container == 'docker') {
    msg = "Success: docker has been enabled and started."
  } else if (container == 'balena') {
    msg = "Success: balena has been enabled and started."
  } else if (container == 'none') {
    msg = "Success: docker and balena have been disabled and stopped."
  } else {
    msg = "Success: docker and balena have been enabled and started."
  }

  exec(command, (error, stdout, stderr) => {
    if (error || stderr) {
      console.log('Error: ' + error + " - " + stderr);
      return;
    }
    console.log(msg);
  });
}